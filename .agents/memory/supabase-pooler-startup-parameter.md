...
**How to apply:** Keep connection options limited to supported startup settings, and configure `statement_timeout`/`lock_timeout` after connection (preferably `SET LOCAL` inside a transaction for destructive work). In one-off `pg` runners, use a validated static SQL literal for `SET`; PostgreSQL does not accept bind placeholders there.