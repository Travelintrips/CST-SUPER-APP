...
**How to apply:** Fail with an explicit source-gross mismatch before the generic net error, and repair/reverse the stale journal or correct the live payment only after the underlying payment evidence is verified. Never override the bank amount.