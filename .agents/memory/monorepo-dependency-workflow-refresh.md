...
After a lockfile-based pnpm synchronization, restart every affected artifact workflow before judging dependency resolution or preview behavior. Existing Vite processes can retain the old package graph and continue reporting missing packages even though the artifact symlinks are repaired. If a declared package is still missing, a frozen-lockfile install may be required before restarting.
