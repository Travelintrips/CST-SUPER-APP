...
**Why:** Artifact workflows (api-server, bizportal, customer-portal, logistic-order, cst-driver, mockup-sandbox) *can* auto-register mid-session (contradicts an earlier note that claimed they never do — that claim was wrong, just unpredictable timing). Once they exist, Gateway must NOT also try to start those same services.
