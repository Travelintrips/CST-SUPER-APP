  ...
  return (
    <a
      ...
      style={{
        ...
        boxShadow: "0 4px 20px rgba(37,211,102,0.50), 0 2px 8px rgba(0,0,0,0.14)",
        WebkitTapHighlightColor: "transparent",
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow =
          "0 8px 28px rgba(37,211,102,0.60), 0 4px 12px rgba(0,0,0,0.16)";
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow =
          "0 4px 20px rgba(37,211,102,0.50), 0 2px 8px rgba(0,0,0,0.14)";
      }}
    >
      <WaSvg />
    </a>
...
export function WhatsAppButton() {
  const { data: company, isPending } = useGetPortalCompany({
  ...
  const phone = raw ? normalizePhone(raw) : FALLBACK_PHONE;
  return <WaButtonAnchor phone={phone} />;
}

export function WhatsAppButtonStandalone({ defaultPhone }: { defaultPhone?: string }) {
  const { data: company, isPending } = useGetPortalCompany({
    query: { queryKey: ["getPortalCompany"], staleTime: 5 * 60 * 1000 },
  });
  ...
  const raw = company?.phone ?? defaultPhone ?? FALLBACK_PHONE;
