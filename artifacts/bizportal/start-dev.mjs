...
const GW_PORT = Number(process.env.BIZPORTAL_PORT || process.env.PORT || 6800);
const VITE_PORT = Number(process.env.BIZPORTAL_VITE_PORT || 18442);

process.env.PORT = String(VITE_PORT);
process.env.BASE_PATH = process.env.BASE_PATH || "/bizportal/";
...
srv.listen(GW_PORT, "0.0.0.0", () => {
  console.log(`[PORT CHECK] PID=${process.pid} PORT=${GW_PORT} SERVICE=bizportal`);
...
vite.on("exit", (code) => {
  console.log(`[bizportal] vite exited (${code})`);
  srv.close();
  process.exit(code ?? 1);
});
